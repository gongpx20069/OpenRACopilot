...
def test_camera_operations(game_api: GameAPI):
    ...
    try:
        ...
        logger.info(f"📷 移动相机到位置: {location}")
        game_api.move_camera_by_location(location)
        logger.info("✅ 相机位置移动成功")
        time.sleep(1)
        
        logger.info("📷 移动相机到单位位置")
        sample_actor = Actor(328)
        game_api.update_actor(sample_actor)
        logger.info(sample_actor.position)
        ...
        # 测试按方向移动相机
        logger.info("📷 向上移动相机10个单位")
        game_api.move_camera_by_direction("上", 10)
        logger.info("✅ 相机方向移动成功")
        time.sleep(1)
        
        # 测试向右移动相机
        logger.info("📷 向右移动相机15个单位")
        game_api.move_camera_by_direction("右", 15)
        logger.info("✅ 相机方向移动成功")
        time.sleep(1)
        
        logger.info(" 向左下移动13个单位")
        game_api.move_camera_by_direction("左下",13) 
        logger.info("✅ 相机方向移动成功")
        time.sleep(1)
        
    except GameAPIError as e:
        logger.info(f"❌ 相机操作失败: {e.code} - {e.message}")
    except Exception as e:
        logger.info(f"❌ 相机操作异常: {e}")
    ...
    try:
        ...
        if game_api.ensure_can_produce_unit("步兵"):
            ...
            if wait_id:
                success = game_api.wait(wait_id, 15.0)
                if success:
                    logger.info(f"✅ 步兵生产完成")
                else:
                    logger.info(f"⏰ 步兵生产超时")
            else:
                logger.info(f"❌ 步兵生产任务创建失败")
            time.sleep(1)
        else:
            logger.info("❌ 无法生产步兵")
        ...
        if infantry_units:
            ...
            for i, infantry in enumerate(infantry_units):
                logger.info(f"  {i+1}. ID:{infantry.actor_id} 位置:({infantry.position.x},{infantry.position.y}) HP:{infantry.hppercent}%")
        
    except GameAPIError as e:
        logger.info(f"❌ 建筑操作失败: {e.code} - {e.message}")
    except Exception as e:
        logger.info(f"❌ 建筑操作异常: {e}")
    ...
    try:
        ...
        if actors:
            ...
            for i, actor in enumerate(actors[:5]):  # 只显示前5个
                logger.info(f"  {i+1}. ID:{actor.actor_id} 类型:{actor.type} 位置:({actor.position.x},{actor.position.y}) HP:{actor.hppercent}%")
            if len(actors) > 5:
                logger.info(f"  ... 还有 {len(actors)-5} 个单位")
        ...
        logger.info(f"❌ 查询操作失败: {e.code} - {e.message}")
    except Exception as e:
        logger.info(f"❌ 查询操作异常: {e}")
...
def test_production_operations(game_api):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"❌ 查询生产队列失败: {e}")
        
    except GameAPIError as e:
        logger.info(f"❌ 生产操作失败: {e.code} - {e.message}")
    except Exception as e:
        logger.info(f"❌ 生产操作异常: {e}")
...
def test_unit_movement(game_api):
    ...
    try:
        ...
        if actors:
            logger.info(f"🎯 找到 {len(actors)} 个步兵单位")
            
            # 选择第一个步兵进行移动测试
            test_actor = actors[0]
            logger.info(f"🎯 测试单位: ID={test_actor.actor_id}, 位置=({test_actor.position.x}, {test_actor.position.y})")
            
            # 移动到当前位置附近
            target_location = Location(test_actor.position.x + 5, test_actor.position.y + 5)
            logger.info(f"🎯 移动到目标位置: ({target_location.x}, {target_location.y})")
            
            game_api.move_units_by_location([test_actor], target_location)
            logger.info("✅ 移动命令发送成功")
            ...
            logger.info("❌ 没有找到可移动的单位")
        
    except GameAPIError as e:
        logger.info(f"❌ 单位移动失败: {e.code} - {e.message}")
    except Exception as e:
        logger.info(f"❌ 单位移动异常: {e}")

def main():
    """主函数"""
    logger.info("🚀 开始真实GameAPI测试")
    logger.info("=" * 60)
    
    # 测试服务器连接
    if not test_server_connection():
        logger.info("❌ 服务器连接失败，测试终止")
        return
    
    # 创建GameAPI实例
    try:
        game_api = GameAPI("localhost", 7445, "zh")
        logger.info("✅ GameAPI实例创建成功")
    except Exception as e:
        logger.info(f"❌ GameAPI实例创建失败: {e}")
        return
    time.sleep(2)
